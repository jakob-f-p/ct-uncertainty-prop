...
auto ModelingWidget::SyncSpinBoxes() const -> void {
...
DataSourceWidget::DataSourceWidget(CtStructureTree& ctStructureTree, CtDataSource& dataSource, QWidget* parent) :
    ...
    connect(SelectSourceTypeButtonGroup, &QButtonGroup::buttonClicked, this, [this](QAbstractButton* button) {
        ...
        auto const* oldWidget = StackedWidget->currentWidget();
        if (auto const* newWidget = std::visit([](auto* widget) { return static_cast<QWidget*>(widget); },
                                               newWidgetVariant);
            newWidget == oldWidget)
            return;
        ...
        if (std::holds_alternative<CtDataImportWidget*>(newWidgetVariant)) {
            ...
            if (auto const success = importWidget->Prepare(); !success)
                return;
        }
    ...
    bool const isImplicit = dynamic_cast<ImplicitCtDataSource*>(&dataSource);
...
auto DataSourceWidget::UpdateDataSource(CtDataSource& dataSource) const -> void {
    bool const isImplicit = dynamic_cast<ImplicitCtDataSource*>(&dataSource);
    if (!isImplicit && !dynamic_cast<NrrdCtDataSource*>(&dataSource))
        throw std::runtime_error("invalid data source type");
...
void CtStructureTreeWidget::DisableButtons() const {
...
void CtStructureTreeWidget::UpdateButtonStates(QItemSelection const& selected, QItemSelection const& /*unused*/) const {
    ...
    const bool isBasicStructure = selectedIndex.isValid()
                                  && selectedIndex.data(IS_BASIC_STRUCTURE).toBool();
...
auto CtStructureTreeWidget::GetCtDataSource() const -> CtDataSource& { return *DataSource; }
...
auto CtDataImportWidget::GetCtDataSource() const -> CtDataSource& { return *DataSource; }
